...
![](nvt_media/b694e05fa9c24f47e6e29c5cae45d71b.png)
...
![](nvt_media/bed26e3482a60f679e613393cd3891ed.png)

![](nvt_media/5cd7beb9a707ea9d8b60abd1a42aed6c.png)
...
![](nvt_media/6bb8b5d1a6631f1c5f6cae8dba635aff.png)
...
![](nvt_media/154994a654c6dad4c3cc4f07bb972cfc.png)
...
![](nvt_media/a31a0573cf1be0a4e0439befdccb98f7.png)
...
![](nvt_media/4dd7c24cad539513735955c0a426be35.png)
...
1.  Get the AD information, and organize this information as host, vi and channel information, and set them to the capture.
...
1.  Polling and notify the channel video norm switch.
...
1.  Polling and notify the channel video loss.
...
## Hardware Device Abstraction Layer (HDAL)
...
![](nvt_media/cccc9381a8455a6aa60ac23dea320ea7.png)
...
![](nvt_media/2b56a44a0ce5ac5e18bbf296ec727dcd.png)
...
1.  Initiate the capture and the encoder modules.

| if ((ret = hd_videocap_init()) != HD_OK) {  return ret; } if ((ret = hd_videoenc_init()) != HD_OK) {  return ret; } |
|---------------------------------------------------------------------------------------------------------------------|

1.  Open the capture and the encoder modules.
...
1.  Set the encoder configure and parameters. **The data_pool (the buffer name is enc_out_ddr0) is allocated from the HD_COMMON_MEM_ENC_OUT_POOL. And only one data_pool is valid for the encoder, so data_pool[1\~3].mode must set to disable.**
...
1.  Bind the capture and the encoder modules, and then the stream will flow from the capture to the encoder.

| ret = hd_videocap_bind(SAMPLE_VCAP_OUT_ID, SAMPLE_VENC_IN_ID); if (ret != HD_OK) {  printf("videocap bind fail\\n");  goto exit; } |
|------------------------------------------------------------------------------------------------------------------------------------|

1.  Start the capture and the encoder modules.
...
1.  Create a thread to encode the stream from the capture. The main procedure in this thread is using the *hd_videoenc_poll_list()* API to check the stream is ready or not. If the stream is ready, use *hd_videoenc_recv_list()* API to receive it.
...
1.  Wait the key press ‘q’ to exit the thread.
2.  Stop the capture and the encoder modules.
...
1.  Unbind the capture and the encoder modules.

| if (HD_OK != hd_videocap_unbind(SAMPLE_VCAP_OUT_ID)) {  printf("hd_videocap_unbind fail\\n"); } |
|-------------------------------------------------------------------------------------------------|

1.  Close the capture and the encoder modules.
...
![](nvt_media/d77c89a76528fef74e43960a8cec8fa1.png)
...
1.  Start the OSG module. After starting the module, the OSG image will paste on the encoded stream.

| ret = hd_videoenc_start(streams.stamp_path_id[idx]); if (ret != HD_OK) { printf("start enc stamp fail = %d\\n", ret); goto exit; } |
|------------------------------------------------------------------------------------------------------------------------------------|

1.  Run the *encode_thread()* and wait ‘q’ key to exit the program.
2.  Stop the OSG module.

| if (HD_OK != hd_videoenc_stop(streams.stamp_path_id[idx])) { printf("stop enc stamp [%ld] fail\\n", idx); } |
|-------------------------------------------------------------------------------------------------------------|

1.  Close the OSG stamp module.

| if ((ret = hd_videoenc_close(p_streams-\>stamp_path_id[i])) != HD_OK) { return ret; } |
|---------------------------------------------------------------------------------------|

1.  **encode_with_mask.c**

Stream flow:

![](nvt_media/ef5d9595dd7cf58b0202184c5fc1fba9.png)
...
1.  Set the parameters of the capture mask and the encode mask.
...
1.  Start the modules of the capture mask and the encode mask. After starting these modules, the mask images will paste on the encoded stream.
...
1.  Run the *encode_thread()* and wait ‘q’ key to exit the program. And you can press ‘e’ key to use *hd_videocap_start()*/*hd_videocap_stop()* to switch the capture mask to ON or OFF. And you can also press ‘y’ key to use *hd_videoenc_set()* to update the position of the encode mask, and then use *hd_videoenc_start()* to activate the new position parameters.
2.  Stop the modules of the capture mask and the encode mask.
...
1.  Close the modules of the capture mask and the encode mask.
...
![](nvt_media/23775f684d92c6936c2289182ca10171.png)
...
![](nvt_media/b7ad50db990302675cabbf7cf8c5f5fc.png)
...
1.  Initiate the capture, the video process and the video out modules.
...
1.  Set the parameters of the video out module.
...
1.  Bind the capture, the video process and the video out modules, and then the video stream will flow from the capture to the video out.
...
1.  Start the capture, the video process and the video out modules. The different between *hd_videoXXX_start_list()* and *hd_videoXXX_start()* is that *hd_videoXXX_start_list()* will start a “list” of XXX modules.
...
1.  Unbind the capture, the video process and the video out modules.
...
1.  Close the capture, the video process and the video out modules.
...
1.  Un-initiate the capture, the video process and the video out modules.
...
![](nvt_media/a09e2ade5edc7467d2eb3114bb5cd0ae.png)
...
![](nvt_media/1b32ed82a375f85adf158a1d245ca301.png)
...
1.  Initiate the video decoder, the video process, the video out and the video encoder modules.
...
1.  Open the video decoder, the video process, the video out and the video encoder modules. The video process 0 \~ (*MAX_BITSTREAM_NUM* – 1) are used for the bitstream decode to the video out, and the video process *MAX_BITSTREAM_NUM* is used for the bitstream decode to the video encoder for JPEG.
...
1.  Set the video process configure and parameters.
...
1.  Bind the video decoder, the video process and the video out modules, and then the video stream will flow from the decoder to the video out.
...
1.  Start the video decoder, the video process and the video out modules.
...
1.  Create a thread to read the h.265 bitstream files, and then uses *hd_videodec_send_list()* to send the bitstream to the decode module. The YUV output from the decode module will flow to the video out module via the video process module.
...
1.  If you press ‘y’ key, it will toggle single window and 2x2 grid windows by changing the parameters of the video process and the video out modules.
2.  If you press ‘p’ key, it will snapshot one JPEG file. **This sample will demo how to use the push/pull APIs to scale and encode a single frame.** Compare with the binding mode, this single mode must prepare the input/output buffer of the modules first.
...
1.  Wait the key press ‘q’ to exit the thread.
2.  Stop the video decoder, the video process and the video out modules.
...
1.  Unbind the video decoder, the video process and the video out modules.
...
1.  Close the video decoder, the video process, the video out and the video encoder modules.
...
1.  Un-initiate the video decoder, the video process, the video out and the video encoder modules.
...
![](nvt_media/0c4de57c179c9aaa4a4aad4e25e300a5.png)
...
![](nvt_media/7c4e05717ecfc666ba5a57c10dd67a7b.png)
...
1.  Initiate the capture, the video process, the video out and the video encoder modules.
...
1.  Open the capture, the video process, the video out and the video encoder modules. 0 \~ (*MAX_BITSTREAM_NUM* – 1) modules are used for capturing the video frames and display on LCD, and *MAX_BITSTREAM_NUM* modules are used for encoding the video out to be as h.265 file.
...
1.  First, bind the capture, the video process and the video out modules, and then the video stream will flow from the capture to the video out. Second, bind the video out, the video process and the video encoder modules, and then the video stream will flow from the video out to the video encoder module.
...
1.  Start the capture, the video process, the video out and the video encoder modules.
...
1.  Create a thread to encode the stream from the video output. The main procedure in this thread is using the *hd_videoenc_poll_list()* API to check the stream is ready or not. If the stream is ready, use *hd_videoenc_recv_list()* API to receive it.
...
1.  Wait the key press ‘q’ to exit the thread.
2.  Stop the capture, the video process, the video out and the video encoder modules.
...
1.  Close the capture, the video process, the video out and the video encoder modules.
...
1.  Un-initiate the capture, the video process, the video out and the video encoder modules.
